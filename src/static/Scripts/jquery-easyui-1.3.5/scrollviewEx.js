...
var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    renderRow: function(target, fields, frozen, rowIndex, rowData)
    {
        if(rowData)
        {
            ...
            if(frozen && opts.rownumbers)
            {
                ...
                if(opts.pagination)
                {
                    rownumber += (opts.pageNumber - 1) * (this.PAGE_SIZE && this.PAGE_SIZE > 0 ? this.PAGE_SIZE : opts.pageSize);
                }
            ...
            for(var i = 0; i < fields.length; i++)
            {
                ...
                if(col)
                {
                    ...
                    var value = GetRowValue(rowData, field);    //field支持子属性--XJW 2015.12.31
    ...
    onBeforeRender: function(target)
    {
        ...
        function scrolling()
        {
            ...
            } else
            {
                ...
                var top = topDiv.position().top + topDiv._outerHeight() - headerHeight - 1;//Edit By XJW 修改缩放时无限刷新的BUG   //var top = topDiv.position().top + topDiv._outerHeight() - headerHeight;
                var bottom = bottomDiv.position().top - headerHeight - 1;
                ...
                {
                    var page = Math.floor(this.index / (view.PAGE_SIZE && view.PAGE_SIZE > 0 ? view.PAGE_SIZE : opts.pageSize));
                    this.getRows.call(this, target, page, function(rows)
                    {
                        this.r2 = this.r1;
                        this.r1 = rows;
                        this.index = (page - 1) * (view.PAGE_SIZE && view.PAGE_SIZE > 0 ? view.PAGE_SIZE : opts.pageSize);
                        this.rows = this.r1.concat(this.r2);
                        this.populate.call(this, target);
                    });
                } else if(bottom < dc.body2.height())
                {
                    var page = Math.floor(this.index / (view.PAGE_SIZE && view.PAGE_SIZE > 0 ? view.PAGE_SIZE : opts.pageSize)) + 2;
                    if(this.r2.length)
                    {
                        page++;
                    }
                    this.getRows.call(this, target, page, function(rows)
                    {
                        ...
                        } else
                        {
                            this.r1 = this.r2;
                            this.r2 = rows;
                            this.index += (view.PAGE_SIZE && view.PAGE_SIZE > 0 ? view.PAGE_SIZE : opts.pageSize);
                        }
                        this.rows = this.r1.concat(this.r2);
                        this.populate.call(this, target);
                    });
            ...
            function reload()
            {
                ...
                var page = Math.floor(index / (view.PAGE_SIZE && view.PAGE_SIZE > 0 ? view.PAGE_SIZE : opts.pageSize)) + 1;
    ...
    getRows: function(target, page, callback)
    {
        ...
        var opts = state.options;
        var index = (page - 1) * (this.PAGE_SIZE && this.PAGE_SIZE > 0 ? this.PAGE_SIZE : opts.pageSize);

        if(opts.onBeforeFetch.call(target, page) == false) { return }

        var rows = state.data.firstRows.slice(index, index + (this.PAGE_SIZE && this.PAGE_SIZE > 0 ? this.PAGE_SIZE : opts.pageSize));
        if(rows.length)
        {
        ...
        {
            if(page > 0 && rows.length == 0) { return; }

            var param = $.extend({}, opts.queryParams, {
                page: page,
                rows: (this.PAGE_SIZE && this.PAGE_SIZE > 0 ? this.PAGE_SIZE : opts.pageSize)
            });
            if(opts.sortName)
            {
                $.extend(param, {
    ...
    populate: function(target)
    {
        ...
        if(this.rows.length)
        {
            ...
            body.children('div.datagrid-btable-bottom').css('height', (this.REAL_ROWS_TOTAL && this.REAL_ROWS_TOTAL > 0 ? this.REAL_ROWS_TOTAL : state.data.total) * rowHeight - this.rows.length * rowHeight - this.index * rowHeight);
            ...
            opts.onLoadSuccess.call(target, {
                total: (this.REAL_ROWS_TOTAL && this.REAL_ROWS_TOTAL > 0 ? this.REAL_ROWS_TOTAL : state.data.total),
                rows: this.rows
            });
        }
    }
...
$.extend($.fn.datagrid.methods, {
    ...
    keyBoardSrcoll: function(jq, index)
    {
        //获得行
        var $tr = $.data(jq[0], 'datagrid').options.finder.getTr(jq[0], 0, 'body', 1),
            trHeight = $tr.outerHeight(),
            bodyHeight = $tr.parents("div.datagrid-body").height(),
        //计算可见区域行数
            viewCount = bodyHeight / trHeight,
        //可见第一行
            viewIndex = parseInt(index + viewCount * 0.5);
        jq.datagrid("scrollTo", viewIndex);
        setTimeout(function()
        {
